...
def test_and():
    ...
    assert tree.decide_query("C") is True
    assert tree.decide_query("F") is False
...
def test_and_in_conclusions():
    ...
    parser = Parser(content)
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("F") is True
    assert tree.decide_query("C") is True
    assert tree.decide_query("D") is True
    assert tree.decide_query("U") is True
...
def test_mix():
    ...
    parser = Parser(content)
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("G") is True
    assert tree.decide_query("T") is False
    assert tree.decide_query("X") is False
...
def test_multiple_initial_facts():
    ...
    parser = Parser(content)
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("E") is True
    assert tree.decide_query("F") is False
...
def test_multiple_initial_facts2():
    ...
    parser = Parser(content)
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("E") is False
    assert tree.decide_query("F") is False
...
def test_multiple_no_initial_facts1():
    ...
    parser = Parser(content)
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("C") is False
...
def test_not():
    ...
    parser = Parser(content)
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("B") is False
    assert tree.decide_query("D") is True
...
def test_parenthesis():
    ...
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("C") is True
    assert tree.decide_query("D") is False
    assert tree.decide_query("W") is False
    assert tree.decide_query("G") is True
    assert tree.decide_query("F") is True
    assert tree.decide_query("Z") is True
...
def test_xor():
    ...
    assert tree.decide_query("C") is False
    assert tree.decide_query("F") is True
    assert tree.decide_query("I") is True
    assert tree.decide_query("L") is False
...
def test_test_and1():
    ...
    parser = Parser(content)
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("A") is True
    assert tree.decide_query("G") is True
    assert tree.decide_query("K") is True
    assert tree.decide_query("P") is True
...
def test_test_and2():
    ...
    assert tree.decide_query("A") is True
    assert tree.decide_query("G") is True
    assert tree.decide_query("K") is False
    assert tree.decide_query("P") is True
...
def test_test_or1():
    ...
    parser = Parser(content)
    tree = RPNTree(parser.atoms, parser.rpn_rules, parser.facts)
    assert tree.decide_query("A") is False
...
def test_test_paran11():
    try:
        with open('./tests/_correction/test_paran11') as f:
            content = f.readlines()
    except:
        print('Error opening file for reading')
        raise
